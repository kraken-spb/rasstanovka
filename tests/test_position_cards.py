...
class PositionCardsTest(unittest.TestCase):
    ...
    def people(self, cards):
        return {worker['full_name']: worker for card in cards for shift in card['shifts']
                for group in shift['groups'] for work in group['works'] for worker in work['workers']}
    ...
    def test_snapshot_grouping_responsibility_and_history(self, render):
        with self.module.app.app_context():
            db = self.module.get_db()
            db.execute('INSERT INTO crew_members(crew_id,worker_id) VALUES (?,?)',
                       (self.fixture.other_crew_id, self.fixture.visible_worker))
            db.commit()
        ...
        self.assertEqual(group['crew_id'], self.fixture.crew_id)
        self.assertEqual(group['crew_name'], 'Экспорт')
    ...
    @patch('position_cards.build_position_cards_pdf', return_value=b'%PDF-fixture')
    def test_effective_category_overrides_raw_value_and_blank_stays_unknown(self, render):
        self.assertEqual(self.export(category='Ручная ГДЛР', shift='1 смена').status_code, 200)
        workers = self.people(render.call_args.args[0])
        self.assertEqual(list(workers), ['=Формула'])
        self.assertEqual(workers['=Формула']['category'], 'Ручная ГДЛР')
        with self.module.app.app_context():
            db = self.module.get_db()
            db.execute("UPDATE workers SET category='' WHERE id=?", (self.fixture.legacy_worker,))
            db.commit()
        self.assertEqual(self.export(category='', shift='2 смена').status_code, 200)
        workers = self.people(render.call_args.args[0])
        self.assertEqual(list(workers), ['Ночной сотрудник'])
        self.assertEqual(workers['Ночной сотрудник']['category'], '')
    ...
    def test_authorization_legacy_and_explicit_scope(self, render):
        ...
        self.assertEqual(self.people(render.call_args.args[0])['=Формула']['category'], 'Ручная ГДЛР')
        self.assertEqual(self.people(render.call_args.args[0])['=Формула']['employer'], '=Работодатель')
        self.assertEqual(self.people(render.call_args.args[0])['Ночной сотрудник']['employer'], 'Работодатель ночи')
        with self.module.app.app_context():
            db = self.module.get_db()
        ...
        for client in (self.admin, self.fixture.foreman):
            ...
            self.assertEqual(self.people(render.call_args.args[0])['Чужой сотрудник']['employer'],
                             'Чужой работодатель')
            self.assertEqual(self.people(render.call_args.args[0])['Чужой сотрудник']['category'],
                             'Исходная ГДЛР')
            self.assertEqual(self.export(client, department='СМУ А').status_code, 404)

    @patch('position_cards.build_position_cards_pdf', return_value=b'%PDF-fixture')
    def test_empty_assignment_employer_stays_unknown(self, render):
        with self.module.app.app_context():
            db = self.module.get_db()
            db.execute('UPDATE assignments SET employer=\'\' WHERE worker_id=?',
                       (self.fixture.visible_worker,))
            db.commit()
        self.assertEqual(self.export(department='СМУ А', shift='1 смена').status_code, 200)
        worker = self.people(render.call_args.args[0])['=Формула']
        self.assertEqual(worker['employer'], '')
    ...
    def test_permissions_validation_and_empty_date(self):
        ...
        viewer_pdf = self.export(self.fixture.viewer)
        self.assertEqual(viewer_pdf.status_code, 200)
        self.assertEqual(viewer_pdf.headers['X-Export-Count'], '4')
