...
class PlacementReportTest(unittest.TestCase):
    ...
    def test_distinct_workers_three_exclusive_statuses_and_no_writes(self):
        ...
        self.assertEqual(data['totals'], {'total':5,'assigned':1,'unassigned':3,'absent':1})
    ...
    def test_filters_blank_values_and_effective_category(self):
        with self.module.app.app_context():
            db = self.module.get_db()
            category = db.execute("INSERT INTO gdlr_categories(name,name_key,edit_token,updated_by,updated_at) VALUES ('Новая категория','новая категория','t',?,'now')", (self.admin,)).lastrowid
            db.execute("INSERT INTO employee_gdlr(worker_id,category_id,edit_token,updated_by,updated_at) VALUES (?,?,'t',?,'now')", (self.people[0],category,self.admin))
            db.commit()
    ...
    def test_inactive_roster_excluded_but_historical_assignments_kept(self):
        ...
        self.assertEqual(empty_date['totals'], {'total':3,'assigned':0,'unassigned':3,'absent':0})
    ...
    def test_permissions_dates_and_smu_scope(self):
        ...
        self.assertEqual(self.get(self.viewer).get_json()['totals']['total'],5)
        ...
        with self.module.app.app_context():
            ...
            db.commit()
        self.assertEqual(self.get(self.foreman).get_json()['totals']['total'],2)
