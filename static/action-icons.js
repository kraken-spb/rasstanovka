(() => {
  ...
  function decorate(control) {
    ...
    const retainLabel = Boolean(control.dataset.view || control.closest('dialog, .login-card, .multi-filter-dialog, .transfer-window, .staffing-toolbar-actions-content, .mobile-panel-content, .mobile-navigation-items, .mobile-disclosure') || control.matches('[data-summary-mode], [data-catalog], [data-display]'));
