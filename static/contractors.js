(() => {
  ...
  const visible = () => $('view-contractors').classList.contains('active');
  const hasDrafts = () => state.drafts.size || state.createDraft.length > 0;
  function status(message, isError = false) {
    const node = $('contractors-status'); node.textContent = message; node.classList.toggle('error-text', isError);
  }
  ...
  function render() {
    ...
    $('contractors-count').textContent = count.toLocaleString('ru-RU');
    $('contractors-bound').textContent = bound.toLocaleString('ru-RU');
    const create = $('contractor-create-name');
    if (create.value !== state.createDraft) create.value = state.createDraft;
    const list = $('contractor-catalog-list');
    list.replaceChildren(...state.rows.map(item => {
      ...
      const name = el('input', {value: draft.name, maxLength: 200, required: true, 'aria-label': 'Название подрядчика ' + item.name});
      const active = el('input', {type: 'checkbox', checked: draft.active, 'aria-label': 'Доступность подрядчика ' + item.name});
      const save = el('button', {className: 'primary-button', disabled: !state.drafts.has(item.id), onclick: () => saveContractor(item)}, 'Сохранить');
      ...
      return el('article', {className: 'category-catalog-row'}, el('label', {}, 'Название', name),
        el('label', {className: 'check-label'}, active, 'Доступен для выбора'),
        el('small', {}, 'Связано сотрудников: ' + item.employee_count), el('div', {className: 'category-actions'}, save, cancel,
          el('button', {type: 'button', className: 'text-button error-text', 'aria-label': 'Удалить подрядчика ' + item.name,
            onclick: () => remove(item)}, 'Удалить')));
    }));
    $('contractors-empty').hidden = count !== 0;
  }
  async function remove(item) {
    if (!canEdit || state.busy) return;
    if (hasDrafts()) { status('Сохраните или отмените изменения перед удалением.', true); return; }
    if (!confirm('Удалить подрядчика «' + item.name + '» из справочника? Это действие нельзя отменить.')) return;
    state.busy = true; $('view-contractors').inert = true; error(); status('Удаление…');
    try {
      await api('/api/contractors/' + item.id, {method: 'DELETE', body: JSON.stringify({expected_token: item.edit_token})});
      state.rows = state.rows.filter(row => row.id !== item.id);
      render(); status('Подрядчик удалён.');
    } catch (failure) { status('Удаление не выполнено.', true); error(failure.message); }
    finally { state.busy = false; $('view-contractors').inert = false; }
  }
  async function saveContractor(item) {
    const draft = state.drafts.get(item.id);
    if (!canEdit || !draft || state.busy) return;
    state.busy = true; $('view-contractors').inert = true; error(); status('Сохранение подрядчика…');
    try {
      await api('/api/contractors/' + item.id, {method: 'PATCH', body: JSON.stringify(draft)});
      state.drafts.delete(item.id);
      state.rows = (await api('/api/contractors')).rows;
      render(); status('Подрядчик сохранён.');
  ...
  async function create(event) {
    ...
    state.busy = true; $('view-contractors').inert = true; error(); status('Добавление подрядчика…');
    try {
      await api('/api/contractors', {method: 'POST', body: JSON.stringify({name})});
      state.createDraft = ''; state.rows = (await api('/api/contractors')).rows;
      render(); status('Подрядчик добавлен.');
    } catch (failure) { error(failure.message + ' Название осталось в поле.'); }
    finally { state.busy = false; $('view-contractors').inert = false; }
  }
  $('contractor-create-form').addEventListener('submit', create);
  $('contractor-create-name').addEventListener('input', event => { state.createDraft = event.target.value; });
  $('contractors-refresh').addEventListener('click', () => refresh());
  window.addEventListener('beforeunload', event => {
    if (visible() && (state.busy || hasDrafts())) { event.preventDefault(); event.returnValue = ''; }
  });
  window.contractorsScreen = {load, canLeave};
})();
