(() => {
  ...
  const workspace = el('div',{className:'verification-workspace'},
    ...
    el('p',{className:'table-note'},(readOnly ? 'Просмотр статусов проверки и состава сотрудников по позициям.' : 'Проверьте состав сотрудников на каждой позиции. Подтверждение действует для выбранной даты, смены и текущих данных назначения. Изменения требуют повторной проверки.')),
  ...
  function render() {
    ...
    positions.forEach((people,key)=>{
      ...
      if (!readOnly) detail.append(el('div',{className:'verification-position-actions'},verify));
      ...
      people.forEach(row=>{
        ...
        button.addEventListener('click',()=>save([row],action));if (!readOnly) state.append(button);list.append(el('li',{},info,state));
      });
  ...
  async function save(people,action) {
    if(readOnly||busy||!people.length)return;
